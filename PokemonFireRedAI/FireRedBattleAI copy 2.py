...
def get_move_score(move_name,move1,move2,move3,move4,mon,learned):
    ...
    if move_name in ('Selfdestruct','Explosion','Memento'):
        ...
        if get_move_type(move_name) not in move_types:
            if get_move_type(move_name) == 'Normal':
                score += 0.5
            else:
                score += 1
...
def learn_move():
    ...
    inv_img = cv2.cvtColor(np.array(mon_name_img), cv2.COLOR_RGB2BGR)
...
while battle:
    ...
    if get_color(1491,516) == (208,160,208) and make_switch == False:
        ...
        if (opp_mon == 'Diglett' or opp_mon == 'Dugtrio') and ('Flying' not in get_mon_type(my_mon) 
        or my_mon not in ('Gastly','Haunter','Gengar')):
            no_switch = True
        ...
        if my_mon_struggle == True and no_switch == False:
            ...
            if no_moves.find('Foe') != 0 and 'moves left!' in no_moves:
                ...
                for i in range(len(usable_moves)):
                    ...
                    time.sleep(0.5)
                    if get_color(1631,568) == (232,1,1) or my_move_name in (disabled_move,'Selfdestruct','Explosion','Memento','Focus Punch'):
                        pass
                    else:
                        break
            ...
            if no_moves.find('Foe') != 0 and 'moves left!' in no_moves:
                ...
                if make_switch == False:
                    for i in range(len(usable_moves)):
                        ...
                        select_move(move_to_use[0])
                        time.sleep(0.5)
                        if get_color(1631,568) == (232,1,1) or get_move_accuracy(my_move_name) < 70 or my_move_name \
                        in (disabled_move,'Selfdestruct','Explosion','Focus Punch') or (get_move_type(my_move_name) == 'Water' 
                        and opp_mon == 'Lapras') or (get_move_type(my_move_name) == 'Fire' and opp_mon \
                        in ('Growlithe','Arcanine','Ponyta','Rapidash')) or ((move_to_use2[1] in ('Hyper Voice','Uproar') 
                        and opp_mon in ('Voltorb','Electrode','Mr. Mime'))):
                            print(my_move_name + ' is not usable')
                            if i == range(len(usable_moves)) - 1:
                                make_switch = True
                                break
                            else:
                                pass
                        elif my_move_name in ('Recover','Morning Sun','Synthesis','Moonlight') and my_HP <= 50 and opp_move_dmg < 50:
                            break
                        elif move_to_use[2] == 0:
                            if opp_status == '' and opp_mon not in ('Ratatta','Raticate','Machop','Machoke','Machamp'):
                            ...
                            if opp_mon_confused == False:
                                if my_move_name == 'Supersonic' and opp_mon not in ('Voltorb','Electrode','Mr. Mime','Slowpoke','Slowbro','Lickitung'):
                                    ...
                                    break
                            if my_move_name == 'Leech Seed' and LeechSeed == False and 'Grass' not in get_mon_type(opp_mon):
                                    break
                            else:
                                make_switch = True
                                break
                    ...
                    my_actual_move = my_move_name
                # Get status moves
                if make_switch == False and my_move_name not in ('Recover','Morning Sun','Synthesis','Moonlight') and opp_move_dmg/my_HP <= (1/3) and move_to_use[2]/opp_HP < 1 and int(opp_mon_lvl) >= int(my_mon_lvl):
...
# toxic,paralysis,leech seed,sleep,burn,poison,confusion, + move accuracy
                    ...
                    if opp_status == '' and opp_mon_confused == False and LeechSeed == False:
                        for i in range(len(usable_moves)):
                            ...
                            time.sleep(0.5)
                            if get_color(1631,568) == (232,1,1) or get_move_accuracy(status_inflict_move[1]) <= 70:
                                pass
                            elif (opp_status != '' or opp_mon in ('Ratatta','Raticate','Machop','Machoke','Machamp')) and \
                            ...
                            and opp_mon in ('Voltorb','Electrode','Mr. Mime')):
                                ...
                                pass
                            elif move_to_use2[1] in ('PoisonPowder','Toxic','Will-O-Wisp','Glare',
                    ...
                    if check_status_move == False:
                        for i in range(len(usable_moves)):
                            ...
                            time.sleep(0.5)
                            if get_color(1631,568) == (232,1,1) or move_to_use2[1] in (disabled_move,'Memento') or \
                            (move_to_use2[1] == 'Curse' and 'Ghost' in get_mon_type(my_mon)) or \
                            ((move_to_use2[1] in ('Growl','Metal Sound','Screech') and opp_mon in ('Voltorb','Electrode','Mr. Mime'))):
                                pass
    ...
    if make_move == True:
        ...
        # disabled red arrow
        while elapsed < 8 or (get_color(1043,515) == (200,168,72) and get_color(1043,533) == (40,80,104) and get_color(1491,516) != (208,160,208)):
            already_add = False
            ...
            elif move_used.find('Foe') != 0 and (('paralyzed' in move_used and 'it can’t move' in move_used) or 
            ('is fast' in move_used and 'asleep' in move_used) or ('frozen solid' in move_used) or ('flinched' in move_used)):
                my_move_used = False
                if my_move_first == False and opp_move_first == False:
                    my_move_first = True
                ...
                elapsed2 = 0
                while elapsed2 < 3:
                    confused_img = ImageGrab.grab(bbox=(1029, 531, 1747, 656))
                    confused = pyt.image_to_string(confused_img)
                    confused = confused.capitalize()

                    if 'It hurt' in confused:
                        my_move_used = False
                        break
                    elif 'used' in confused:
                        break
                ...
                elapsed2 = 0
                while elapsed2 < 5:
                    my_move_missed_img = ImageGrab.grab(bbox=(1029, 531, 1747, 656))
                    my_move_missed = pyt.image_to_string(my_move_missed_img)
                    my_move_missed = my_move_missed.capitalize()
                    
                    if my_move_missed.find('Foe') == 0 and 'used' in my_move_missed:
                        break
                    ...
                    elif (my_move_missed.find('Foe') != 0 and 'missed' in my_move_missed) or \
                    (my_move_missed.find('Foe') == 0 and ('evaded' in my_move_missed or 'prevents' in my_move_missed)) \
                    or 'failed' in my_move_missed or (my_move_missed.find('Foe') == 0 and 
                    'soundproof' in my_move_missed and 'blocks' in my_move_missed) or \
                    ('doesn’t affect' in my_move_missed and 'foe' in my_move_missed) or \
                    ('no' in my_move_missed and 'target' in my_move_missed) or (my_move_missed.find('Foe') == 0 
                    and 'protected' in my_move_missed):
                        my_move_used = False
                        already_add = True
                        break

                    elif 'fell!' in my_move_missed or 'rose!' in my_move_missed:
                        my_move_used = True
                        ...
                        if my_move_missed.find('Foe') == 0:
                            for i in opp_move_used:
                                if 'NoAdd' not in i:
                                    i.append('NoAdd')
                        if my_move_missed != None or my_move_missed.find('Foe') != 0:
                            break
            ...
            elif 'fainted' in move_used:
                if move_used.find('Foe') == 0:
                    for i in opp_move_used:
                        if 'NoAdd' not in i:
                            i.append('NoAdd')
                if my_move_used != None or move_used.find('Foe') != 0:
                    break
                elif continue_text(move_used_img):
                    pyd.press('z')
            ...
            or ('frozen solid' in move_used))):
                ...
                while elapsed2 < 5:
                    ...
                    if move_missed.find('Foe') != 0 and 'used' in move_missed:
                        break
                    ...
                    elif (move_missed.find('Foe') == 0 and 'missed' in move_missed) or \
                    (move_missed.find('Foe') != 0 and ('evaded' in move_missed or 'prevents' in move_missed)) or \
                    'failed' in move_missed or (move_missed.find('Foe') == 0 and 
                    'soundproof' in move_missed and 'blocks' in move_missed) or \
                    ('doesn’t affect' in move_missed and 'foe' in move_missed) or \
                    ('no' in move_missed and 'target' in move_missed) or (move_missed.find('Foe') != 0
                    and 'protected' in move_missed):
                        already_add = True
                        opp_move_used.append([move_used, False])
                        if move_used not in actual_moves:
                            move_added = True
                            actual_moves.append(move_used)
                        break
                    
                    elif 'fell!' in move_missed or 'rose!' in move_missed:
                        already_add = True
...
# moves that the opp used that affect its stat go in the bottom
        ...
        if get_color(1098,201) == (248,248,216):
            ...
            while get_color(1043,515) == (200,168,72) and get_color(1043,533) == (40,80,104) and get_color(1491,516) != (208,160,208):
                ...
                if after_moves.find('Foe') == 0 and 'cuts' in after_moves and 'attack' in after_moves:
                    my_stat_stages[0] -= 1
                    my_stat_stages[0] = max(min(my_stat_stages[0], 6), -6)

                elif after_moves.find('Foe') != 0 and 'confused!' in after_moves:
                    my_mon_confused = True

                elif move_used.find('Foe') == 0 and 'confused!' in move_used:
                    opp_mon_confused = True

                elif after_moves.find('Foe') == 0 and 'used' in after_moves and '!' in after_moves:
                    move_used = move_used.split('\n')[1]
                    move_used = re.sub('[^a-zA-Z-]', '', move_used)
                    move_used = dfl.get_close_matches(move_used,Gens1to3_Moves,cutoff=0.1)[0]
                ...
                elif 'reflect' in after_moves and 'wore off' in after_moves:
                    reflect_on = 'false'
        
                elif 'light screen' in after_moves and 'wore off' in after_moves:
                    lightscreen_on = 'false'

                elif 'no longer' in after_moves and 'protected' in after_moves:
                    Safeguard = False

                # elif 'make' in after_moves and 'room' in after_moves:

                elif after_moves.find('Foe') != 0 and 'fainted' in after_moves:
                    break
                ...
                elif 'sent' in after_moves and 'out' in after_moves:
                    if my_actual_move in ('Hyper Beam','Frenzy Plant','Blast Burn','Hydro Cannon','Outrage','Petal Dance') and my_move_used == True and opp_mon_faint == True:
                        no_stat_change = True
                        no_new_moves = True
                        opp_stat_stages = [0,0,0,0,0]
                        actual_moves = []
                        move_added = False # change to boolean
...
# if mon faint and faint count is different add
                ...
                elif continue_text(after_moves_img) == True:
                    pyd.press('z')
        ...
        if get_color(1525,431) == (248,248,216):
            ...
            while get_color(1043,515) == (200,168,72) and get_color(1043,533) == (40,80,104):
                    ...
                    # elif 'make' in battle_over and 'room' in battle_over:

                    elif move_used.find('Foe') == 0 and 'confused!' in move_used:
                        opp_mon_confused = True
                    
                    elif battle_over.find('Foe') == 0 and 'used' in battle_over and '!' in battle_over:
                        move_used = move_used.split('\n')[1]
                        move_used = re.sub('[^a-zA-Z-]', '', move_used)
                        move_used = dfl.get_close_matches(move_used,Gens1to3_Moves,cutoff=0.1)[0]
                    ...
                    elif 'reflect' in battle_over and 'wore off' in battle_over:
                        reflect_on = 'false'
            
                    elif 'light screen' in battle_over and 'wore off' in battle_over:
                        lightscreen_on = 'false'
                    
                    elif 'no longer' in battle_over and 'protected' in battle_over:
                        Safeguard = False

                    elif continue_text(battle_over_img) == True:
                        pyd.press('z')
        ...
        if len(opp_move_used) > 0:
            if forced_switch == False and opp_move_used[0][0] == 'Perish Song' and opp_move_used[0][1] == True:
                PerishSong = True
                PerishCount += 1
        if len(opp_move_used) > 1:
            if forced_switch2 == False and opp_move_used[1][0] == 'Perish Song' and opp_move_used[1][1] == True:
                PerishSong = True
                PerishCount += 1
        if len(opp_move_used) > 2:
            if opp_move_used[2][0] == 'Perish Song' and opp_move_used[2][1] == True:
                PerishSong = True
                PerishCount += 1

        if len(opp_move_used) > 0:
            if forced_switch == False and opp_move_used[0][0] in ('Block','Mean Look') and opp_move_used[0][1] == True:
                no_switch = True
        if len(opp_move_used) > 1:
            if forced_switch2 == False and opp_move_used[1][0] in ('Block','Mean Look') and opp_move_used[1][1] == True:
                no_switch = True
        if len(opp_move_used) > 2:
            if opp_move_used[2][0] in ('Block','Mean Look') and opp_move_used[2][1] == True:
                no_switch = True
